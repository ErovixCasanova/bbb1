...
def check_card():
    ...
    def process_payment():
        try:
            ...
            if not client_token_nonce:
                return {'error': 'Failed to get client token nonce'}, 400
            ...
            if response5.status_code != 200:
                return {'error': f'Tokenization failed with status {response5.status_code}: {response5.text}'}, 400
            
            try:
                tokenize_result = response5.json()
            except Exception as e:
                return {'error': f'Failed to parse tokenization response: {str(e)}'}, 400
            
            if not isinstance(tokenize_result, dict):
                return {'error': f'Tokenization response is not a dictionary: {type(tokenize_result)}'}, 400
            
            if 'errors' in tokenize_result:
                return {'error': f'Tokenization errors: {tokenize_result["errors"]}'}, 400
            
            if 'data' not in tokenize_result:
                return {'error': f'Missing "data" key in response: {tokenize_result}'}, 400
            
            if not isinstance(tokenize_result['data'], dict):
                return {'error': f'"data" is not a dictionary: {type(tokenize_result["data"])}'}, 400
            
            if 'tokenizeCreditCard' not in tokenize_result['data']:
                return {'error': f'Missing "tokenizeCreditCard" in data: {tokenize_result["data"]}'}, 400
            
            if not isinstance(tokenize_result['data']['tokenizeCreditCard'], dict):
                return {'error': f'"tokenizeCreditCard" is not a dictionary: {type(tokenize_result["data"]["tokenizeCreditCard"])}'}, 400
            
            payment_token = tokenize_result['data']['tokenizeCreditCard'].get('token')
            if not payment_token:
                return {'error': 'Failed to get payment token'}, 400
            ...
            if not payment_nonce:
                return {'error': 'Missing payment nonce'}, 400
    ...
    if error_message:
        ...
        result = "Approved ✅"
    elif re.search(r'Avs|avs|Nice|Added|Successfully', response_text):
        result = "Approved ✅"
    else:
        if 'declined' in response_text.lower() or 'error' in response_text.lower():
            error_match = re.search(r'Status code \d+: ([^<]+)', response_text)
            if error_match:
